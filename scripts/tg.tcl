...
if { ! [file exists $parinp ] } {
   puts "Note: Input file $parinp containing NAMD \"parameters xxx\" config directives not found."
   if { ! [file exists $ALTNAMDCONFIG] } {
     puts "Error: Could not find a NAMD config file to extract parameter names from"
     exit
   } else {
     puts "Note: Generating $parinp from $ALTNAMDCONFIG"
...
if { [$all_ions num] > 0 } {
  ...
  puts "# Ion chains detected: $ion_chains"
  foreach ic $ion_chains {
    if { $ic in $protein_chains } {
      puts "# Shared protein/ion chain ID: $ic"
      if { $ic == "I" } { 
        puts "# Warning: your topology has a protein chain \"I\"."
        puts "# Chain \"I\" is reserved for initially free ions."
        puts "# The protein chain \"I\" will be assigned a new chainID."
        set fixI 1
      }
    }
  }
  set ion_segnames [lsort -unique [$all_ions get segname]]
  puts "# Ion segnames detected: $ion_segnames"
  if { [llength $ion_segnames] > 1} {
    puts "# All ions will be given segname \"ION\""
  }
...
foreach P $protein_chains {
  ...
  if { $oions_count > 0 } {
    ...
    puts "# Changing segname to \"ION\" for $oions_count ions in protein-chain $P"
  }
  ...
  if { $xw_count > 0 } {
    $xw set chain W
    $xw set segname WX${P}
    puts "# Changing crystal water chain to W and segname to WX${P}"
  }
}
...
if { $pcinum > 0 } {
  ...
  foreach ch [split "ABCDEFGHIJKLMNOPQRSTUVWXYZ" {}] {
    if { $ch in $protein_chains } {
      puts "# searching for new chain ID for old chain I... $ch is taken."
    } else {
      puts "# searching for new chain ID for old chain I... $ch is available."
      lappend available_chainIDs $ch
    }
  }
  puts "# Available chain IDs: [join $available_chainIDs ',']"
  set newch [lindex $available_chainIDs 0]
  puts "# selected $newch"
  $pci set chain $newch
  $pci set segname $newch
} else {
  puts "# No protein/glycan atoms with chainID I found."
}
set ii [atomselect top "ion"]
if { [$ii num] > 0 } {
  set iicids [lsort -unique [$ii get chain]]
  puts "# Ions chainID(s) detected: [join $iicids ',']"
  set inoti [atomselect top "ion and not chain I"]
  if { [$inoti num] > 0 } {
    puts "# Setting chain ID of all atoms selected by macro \"ion\" to I"
    $inoti set chain "I"
  }
  ...
  puts "# ion macro indices: $iindx"
  puts "# segname ION indices: $isndx"
  ...
  if { [llength $diff] > 0 } {
    puts "# Error: some ions selected by macro ion differ from ions selected using segname ION"
    puts "# This means the code needs fixing."
    puts "# ion macro indices: $iindx"
    puts "# segname ION indices: $isndx"
    exit
  }
...
puts "# Using TopoTools to reorder out-of-order fragments"
