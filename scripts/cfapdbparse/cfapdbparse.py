...
#    PostMod['gly_mc_params']=DictFromString(args.glymcparams)
    ...
    fp.write('echo "Running namd2 on vacuum system {} {}; log is run'.format(Base.psf_outfile,post_pdb)+r'${TASK}'+'-1.log"\n')
...
#                fp.write('lay_loop $molid {} [range {} {} 1] {}\n'.format(l.replica_chainID,l.residues[0].resseqnum,l.residues[-1].resseqnum,100))
        ...
        #write_bash_instructions_for_namd(fp,psf,pdb,config='run2.namd',numint=0,numsteps=2*target_numsteps,
        #                                 temperature=temperature,fixed='fixed.pdb',colvars='colvars.pdb')
        fp.write('echo "structure {}" > tmpnamdheader\n'.format(Base.psf_outfile))
        fp.write('echo "coordinates {}" >> tmpnamdheader\n'.format('config.pdb'))
        ...
        fp.write(' > run2.namd\n')
        fp.write('rm tmpnamdheader\n')
        fp.write('echo "Running namd2 SMD on vacuum system {} {}; log is run'.format(Base.psf_outfile,'config.pdb')+r'${TASK}'+'-2.log"\n')
