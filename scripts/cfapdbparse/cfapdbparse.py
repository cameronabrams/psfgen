...
def WritePostMods(fp,psf,pdb,PostMod,Loops,GlycanSegs):
    ...
    if 'do_loop_mc' in PostMod and PostMod['do_loop_mc']:
        ...
        fp.write('   set fa [[atomselect $molid "protein and chain $chain and resid [lindex $l 1] and name CA"]]\n')
        fp.write('   set ca [[atomselect $molid "protein and chain $chain and resid [lindex $l 2] and name CA"]]\n')
        fp.write('   set c [[atomselect $molid "protein and chain $chain and resid [lindex $l 2] and name C"]]\n')
        fp.write('   do_flex_mc $molid $msel $fa $k $ca $co $bg $sigma $epsilon $rcut $nc $temperature [irand_dom 1000 9999] $logid {}\n'.format(logevery))
