...
def WritePostMods(fp,psf,pdb,PostMod,Loops,GlycanSegs):
    ...
    if 'do_loop_mc' in PostMod and PostMod['do_loop_mc']:
        ...
        fp.write('dict set mpc sigma {:.4f}\n'.format(sigma))
        fp.write('dict set mpc epsilon {:.4f}\n'.format(epsilon))
        #fp.write('set r0 1.5\n')
        fp.write('dict set mpc temperature {:.4f}\n'.format(mctemperature))
        fp.write('dict set mpc k {:.4f}\n'.format(mck))
        fp.write('dict set mpc dstop {:.4f}\n'.format(dstop))
        fp.write('dict set mpc sstop {:.4f}\n'.format(sstop))
