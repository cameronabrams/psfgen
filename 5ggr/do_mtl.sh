...
ARGC=$#
i=1
CMM=0.1 ; # concentration of D-mannitol in Molar 
while [ $i -le $ARGC ] ; do
  if [ "${!i}" = "-cs" ]; then
    i=$((i+1))
    CMM=${!i}
...
# 4. solvate
echo "Generating mannitol ($CMM Molar) solvated system..."
vmd -dispdev text -e $PSFGEN_BASEDIR/${PDB}/my_${PDB}_mtl_solv_1.tcl -args -cm $CMM > psfgen_mtl_solv_1.log
packmol < pm-tmp.in
vmd -dispdev text -e $PSFGEN_BASEDIR/${PDB}/my_${PDB}_mtl_solv_2.tcl > psfgen_mtl_solv_2.log

# 5. run NAMD
echo "Running namd2 on solvated system..."
ln -s $PSFGEN_BASEDIR/${PDB}/my_${PDB}_mtl_solv.namd .
ln -s $PSFGEN_BASEDIR/${PDB}/my_${PDB}_mtl_solv_colvars_op.inp .
$CHARMRUN +p16 $NAMD2 my_${PDB}_mtl_solv.namd > mtl_solv.log
